...
if is_voll:
    ...
    #     ax.axvline(d_ref, color="red", linestyle="--", linewidth=1.5)

    # if not np.isnan(p_ref):
    #     ax.axhline(p_ref, color="red", linestyle="--", linewidth=1.5)

    # # Add the red “x” at intersection of reference lines
    # if not np.isnan(d_ref) and not np.isnan(p_ref):
    #     ax.scatter(d_ref, p_ref, color="red", s=60, marker="x", linewidth=2)
    #     legend_entries.append((
    #         f"Reference demand = {d_ref:.2f} GW",
    #         plt.Line2D([], [], color="red", linestyle="--")
    #     ))
    #     legend_entries.append((
    #         f"Reference price = {p_ref:.2f} €/MWh",
    #         plt.Line2D([], [], color="red", linestyle="--", marker="x")
    #     ))
    ...
    #     ax.axvline(d_act, color="green", linestyle="--", linewidth=1.5)
    #     legend_entries.append((
    #         f"Actual demand = {d_act:.2f} GW",
    #         plt.Line2D([], [], color="green", linestyle="--")
    #     ))

    # if not np.isnan(p_act):
    #     ax.axhline(p_act, color="green", linestyle="--", linewidth=1.5)
    #     ax.scatter(d_act, p_act, color="green", s=60, marker="x", linewidth=2)
    #     legend_entries.append((
    #         f"Actual price = {p_act:.2f} €/MWh",
    #         plt.Line2D([], [], color="green", linestyle="--", marker="x")
    #     ))
...
# ---------------------------------------------------------
# Add VOLL tick on y-axis (always)
# ---------------------------------------------------------
...
# Add VOLL if not already included
if not any(abs(t - voll_price) < 1e-6 for t in yticks):
    yticks.append(voll_price)
    yticks = sorted(yticks)
    ax.set_yticks(yticks)

# Now regenerate tick labels safely
ylabels = []
for t in yticks:
    if abs(t - voll_price) < 1e-6:
        ylabels.append("VOLL")
    else:
        ylabels.append(f"{t:.0f}")
...
save_path = os.path.join(output_dir, f"Demand_{ct}_TD{td}_H{h}.pdf")
plt.tight_layout()
